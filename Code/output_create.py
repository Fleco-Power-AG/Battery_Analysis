...
def build_monthly_energy_table(
    ...
    Eigenverbrauch_kWh, Rueckspeisung_PV_kWh, Rueckspeisung_Batterie_kWh,
    Netzbezug_kWh, Eigenverbrauchsquote_Prozent.

    Definitionen (NEU seit 15.9.2026, Beats Frage "wie kann ich EV haben mit
    einem System aus PV und BAT, das sollte noch nicht gehen -- sollte man da
    nicht RS_PV und Rueckspeisung_Bat unterscheiden anstelle EV und RS?"):
      - Eigenverbrauch = Energie, die tatsaechlich an der LAST ankommt:
        ts["pv_ac"] (PV direkt zur Last) + ts["dis_zu_last"] (Batterie-
        Entladung zur Last). Das ist dieselbe last-basierte Definition, die
        battery_optimization.py::allocate_modules() bereits fuer die
        Wirtschaftlichkeits-Tabelle ("Eigenverbrauchsoptimierung", Seite 2)
        verwendet -- dieser Chart nutzte bisher eine ANDERE, ungenauere
        Definition (siehe unten), was zu einem Widerspruch zwischen den
        beiden Seiten fuehren konnte.
      - Rueckspeisung_PV = direkter PV-Export ins Netz, ohne Umweg ueber die
        Batterie (ts["pv_export_direkt"]).
      - Rueckspeisung_Batterie = Batterie-Entladung, die ins Netz exportiert
        wird (ts["dis_zu_export"]).
    ...
    ALTE Definition (bis 15.9.2026, jetzt ersetzt): Eigenverbrauch wurde als
    Residuum "PV-Erzeugung minus Gesamt-Export" berechnet (Rueckspeisung war
    dabei nur EINE Spalte, ts["export"], ohne PV/Batterie-Unterscheidung).
    Bei einem Batterie-System OHNE Last zeigte das faelschlicherweise einen
    Eigenverbrauch > 0, obwohl an keiner Last ueberhaupt etwas ankam: der
    vermeintliche "Eigenverbrauch" war in Wirklichkeit nur der Rundwirkungs-
    grad-Verlust (Laden/Entladen/Trafo) auf dem Weg PV -> Batterie -> Export,
    den das Residuum faelschlich als "vor Ort verbleibende Energie"
    interpretierte. Die Wirtschaftlichkeits-Tabelle auf Seite 2 zeigte fuer
    denselben Fall schon immer korrekt "Eigenverbrauchsoptimierung: 0 CHF",
    da sie bereits die last-basierte Definition nutzte -- dieser Chart war
    die einzige Stelle mit der alten, residual-basierten Definition.
    ...
    # ".get"-artiger Fallback (wie an anderen Stellen in dieser Datei) fuer
    # aeltere ts-DataFrames ohne die granulare PV/Batterie-Aufteilung --
    # pv_export_direkt faellt in dem Fall auf den (undifferenzierten)
    # Gesamt-Export zurueck, die uebrigen Anteile auf 0.
    pv_ac = ts["pv_ac"].values if "pv_ac" in ts.columns else np.zeros(n)
    dis_zu_last = ts["dis_zu_last"].values if "dis_zu_last" in ts.columns else np.zeros(n)
    pv_export_direkt = (
        ts["pv_export_direkt"].values if "pv_export_direkt" in ts.columns else ts["export"].values
    )
    dis_zu_export = ts["dis_zu_export"].values if "dis_zu_export" in ts.columns else np.zeros(n)

    pv_kwh = pd.Series(pv_profile.values * dt_hours, index=idx).groupby(monat).sum()
    eigenverbrauch_kwh = pd.Series((pv_ac + dis_zu_last) * dt_hours, index=idx).groupby(monat).sum()
    rs_pv_kwh = pd.Series(pv_export_direkt * dt_hours, index=idx).groupby(monat).sum()
    rs_bat_kwh = pd.Series(dis_zu_export * dt_hours, index=idx).groupby(monat).sum()
    ...
    rs_pv_kwh = rs_pv_kwh[rs_pv_kwh.index.isin(gueltige_monate)]
    rs_bat_kwh = rs_bat_kwh[rs_bat_kwh.index.isin(gueltige_monate)]
    netzbezug_kwh = netzbezug_kwh[netzbezug_kwh.index.isin(gueltige_monate)]

    # np.errstate: bei einem PV_Bat-System ohne Last oder einem reinen
    # Bat_Last-System (kein PV) ist pv_kwh ueberall bzw. teilweise 0 --
    # np.where wertet trotz des 0.0-Fallbacks BEIDE Zweige aus, was sonst
    # eine harmlose, aber verwirrende "divide by zero"-Konsolenwarnung
    # ausloest.
    ...
    tabelle = pd.DataFrame(
        {
            ...
            "Rueckspeisung_PV_kWh": rs_pv_kwh,
            "Rueckspeisung_Batterie_kWh": rs_bat_kwh,
...
def build_baseline_monthly_energy_table(
    ...
    tabelle = pd.DataFrame(
        {
            "Eigenverbrauch_kWh": eigenverbrauch_kwh,
            "Rueckspeisung_PV_kWh": rueckspeisung_kwh,
            "Rueckspeisung_Batterie_kWh": np.zeros(len(eigenverbrauch_kwh)),
            "Netzbezug_kWh": netzbezug_kwh,
            "Eigenverbrauchsquote_Prozent": eigenverbrauchsquote_pct,
        }
    )
...
# NEU (Beats Frage "wie kann ich EV haben mit einem System aus PV und BAT",
# siehe _plot_eigenverbrauch()/build_monthly_energy_table()): dritte Serie
# fuer die Rueckspeisung-aus-Batterie-Balken, seit die Rueckspeisung in
# RS_PV/RS_Bat aufgeteilt wird. "#0072B2" ist dasselbe CVD-gepruefte Blau wie
# Q2 in _QCOLORS -- unproblematisch, da nie im selben Chart wie die
# Quartalslinien verwendet.
_ACCENT3 = "#0072B2"      # Tertiaerserie "Mit Batterie" (Rueckspeisung Batterie)
_GRAU3 = "#E2E2E2"        # Tertiaerserie "Ohne Batterie" (immer 0, s.u.)
_POS = _ACCENT
_NEG = "#B23B3B"
...
def build_input_summary_rows(
    ...
    def _ruecklieferung_pv_zeilen(schema_key: str, hkn_key: str, floor_key: str, fixtarif_key: str):
        ...
        schema_norm = str(schema_wert).strip().lower() if schema_wert is not None else ""
        # NEU (Beat, 15.9.2026: "Energie-Markup Zeile auf Seite 1 brauche ich
        # nicht"): Zeile "Energie-Markup: Abschlag Lieferung" (nur bei
        # Schema=Spot) entfernt -- der Wert bleibt natuerlich weiter Teil der
        # eigentlichen Tarifberechnung (siehe resolve_rueckliefertarif()),
        # nur die Anzeige hier auf Seite 0 entfaellt.
        if schema_norm == "fixtarif":
    ...
    # NEU seit 15.9.2026 (Beat, Bericht_BAT_standalone.pdf: "wieso sehe ich
    # den Fixtarif bei Bat wenn ich das Schema 'Spot' habe -- das hat ja
    # keinen Einfluss bei Spot"): frueher wurden Batterie-Fixtarif (und auch
    # Batterie-Floor, s.u.) IMMER angezeigt, unabhaengig vom gewaehlten
    # Batterie-Rueckliefertarifschema -- der Fixtarif-Wert war in dem Fall
    # zwar korrekt 0.000 (bzw. was auch immer im Excel steht), aber fachlich
    # bedeutungslos, da resolve_rueckliefertarif() den Fixtarif-Parameter nur
    # verwendet, wenn Schema tatsaechlich "Fixtarif" ist ("Spot ist Spot ohne
    # HKN", siehe dortiger Docstring). Jetzt symmetrisch zur PV-Zeile oben:
    # nur der fuer das jeweils gewaehlte Schema tatsaechlich wirksame Wert
    # wird angezeigt (Spot -> Abschlag Lieferung, Fixtarif -> Fixtarif,
    # RMP_Floor -> Floor). HKN nur anzeigen, wenn er auch tatsaechlich sauber
    # zuordenbar ist: kein Netzbezug fuer die Batterie zulaessig
    # (max_bezug_batterie = 0, d.h. sie kann NUR aus PV geladen werden) UND
    # eine PV-Anlage vorhanden ist (sonst gibt es gar keine PV-Energie, der
    # die HKN zugeordnet werden koennte) UND HKN ueberhaupt Teil der
    # Tarifformel ist (bei Spot nicht, siehe oben).
    def _ruecklieferung_batterie_zeilen(schema_key: str, hkn_key: str, floor_key: str, fixtarif_key: str):
        schema_wert = params.get(schema_key)
        zeilen = [{"label": "Batterie-Schema", "value": _fmt_param(schema_wert, na="unbekannt")}]
        schema_norm = str(schema_wert).strip().lower() if schema_wert is not None else ""
        # NEU (Beat, 15.9.2026: "Energie-Markup Zeile auf Seite 1 brauche ich
        # nicht"): Zeile "Energie-Markup: Abschlag Lieferung" (nur bei
        # Schema=Spot) entfernt, analog zur PV-Zeile oben.
        if schema_norm == "fixtarif":
            zeilen.append({"label": "Batterie-Fixtarif", "value": _fmt_param(params.get(fixtarif_key), "CHF/kWh", 3)})
        if schema_norm == "rmp_floor":
            zeilen.append({"label": "Batterie-Floor", "value": _fmt_param(params.get(floor_key), "CHF/kWh", 3)})
        ...
        if schema_norm in ("fixtarif", "rmp", "rmp_floor") and kein_netzbezug_batterie and pv_vorhanden:
    ...
    rows += _ruecklieferung_batterie_zeilen(
        "rueckliefer_batterie_schema", "rueckliefer_batterie_hkn", "rueckliefer_batterie_floor",
        "rueckliefer_batterie_fixtarif",
    )
...
def build_pdf_report(
    ...
    def _plot_eigenverbrauch(cax, tabelle, farbe1, farbe2, farbe3, einheit_divisor=1.0, einheit_label="kWh", ylim=None):
        # NEU (Beats Frage "wie kann ich EV haben mit einem System aus PV und
        # BAT ... sollte man da nicht RS_PV und Rueckspeisung_Bat
        # unterscheiden anstelle EV und RS?"): drei statt zwei Balken pro
        # Monat -- Eigenverbrauch bleibt last-basiert (siehe
        # build_monthly_energy_table()), Rueckspeisung ist neu nach Quelle
        # (PV direkt / Batterie) aufgeteilt.
        monate_idx = tabelle.index.astype(int)
        labels = [MONATSNAMEN[m - 1] for m in monate_idx]
        x = np.arange(len(labels))
        breite = 0.25
        cax.bar(x - breite, tabelle["Eigenverbrauch_kWh"] / einheit_divisor, breite,
                label="EV", color=farbe1, zorder=3)
        cax.bar(x, tabelle["Rueckspeisung_PV_kWh"] / einheit_divisor, breite,
                label="RS PV", color=farbe2, zorder=3)
        cax.bar(x + breite, tabelle["Rueckspeisung_Batterie_kWh"] / einheit_divisor, breite,
                label="RS Bat", color=farbe3, zorder=3)
        cax.set_xticks(x)
        cax.set_xticklabels(labels, fontsize=6)
        cax.set_ylabel(einheit_label, fontsize=7, color=_TEXT_DIM)
        cax.tick_params(labelsize=6)
        cax.legend(frameon=False, fontsize=6, labelcolor=_TEXT_DIM, loc="upper right", ncol=3,
    ...
    with PdfPages(output_path) as pdf:
        ...
        if nur_batterie:
            ...
            # NEU seit 15.9.2026 (Beats RS_PV/RS_Bat-Vorschlag): Rueckspeisung
            # ist auf zwei Spalten aufgeteilt -- fuer die Eigenverbrauchsquote
            # (Anteil an der PV-Erzeugung, der nicht ins Netz zurueckgeht)
            # zaehlt weiterhin die GESAMTE Rueckspeisung, also beide Spalten
            # zusammen.
            total_ev = float(monatstabelle["Eigenverbrauch_kWh"].sum())
            total_ev_ohne = float(monatstabelle_ohne["Eigenverbrauch_kWh"].sum())
            total_rs = float(
                monatstabelle["Rueckspeisung_PV_kWh"].sum() + monatstabelle["Rueckspeisung_Batterie_kWh"].sum()
            )
            total_rs_ohne = float(
                monatstabelle_ohne["Rueckspeisung_PV_kWh"].sum()
                + monatstabelle_ohne["Rueckspeisung_Batterie_kWh"].sum()
            )
            ...
            # bleiben.
            max_monatswert_kwh = max(
                monatstabelle["Eigenverbrauch_kWh"].max(),
                monatstabelle["Rueckspeisung_PV_kWh"].max(),
                monatstabelle["Rueckspeisung_Batterie_kWh"].max(),
                monatstabelle_ohne["Eigenverbrauch_kWh"].max(),
                monatstabelle_ohne["Rueckspeisung_PV_kWh"].max(),
                monatstabelle_ohne["Rueckspeisung_Batterie_kWh"].max(),
            )
            ...
            zeilen_zweispaltig = [
                {
                    ...
                    "ohne": {"plot": lambda cax: _plot_eigenverbrauch(
                                 cax, monatstabelle_ohne, _GRAU, _GRAU2, _GRAU3, einheit_divisor, einheit_label,
                                 ylim=ev_ylim),
                             "caption": f"Eigenverbrauchsquote: {quote_ohne:,.0f} %"},
                    "mit": {"plot": lambda cax: _plot_eigenverbrauch(
                                cax, monatstabelle, _ACCENT, _ACCENT2, _ACCENT3, einheit_divisor, einheit_label,
                                ylim=ev_ylim),
