...
Testen besonders zu achten ist (Kommentare an den jeweiligen Stellen im Code):
  ...
  - `om.objective.expr = om.objective.expr + ...` zum Erweitern der
    ...
    mehr mehrfach `+=` -- Root-Cause-Fix eines echten CBC-Absturzes
    "duplicates in objective and matrix" / "termination condition: unknown",
    gefunden ueber Beats Solver-Log vom 10.9.2026 mit Peak-Shaving + SRL
    "optimiert" gleichzeitig aktiv, siehe ausfuehrlicher Kommentar in main())
...
def main(
    ...
    # weiterreicht (OHNE dass ueberhaupt eine Optimierung stattfand) --
    # mutmasslich exakt der seit laengerem unklare, wiederholt aufgetretene
    # "termination condition: unknown"-Absturz (siehe Bugfix-Log/
    # Projektnotizen), jetzt zumindest eingegrenzt: 105119 ist fast exakt
    # 3 * 35040 (die Anzahl Zeitschritte) -- alle DREI zeitreihenbasierten
    # Flow-Kosten (Bezugstarif, PV- und Batterie-Rueckliefertarif) tauchen
    # verdoppelt im LP-File auf, obwohl HIER NUR EINE EINZIGE
    # `om.objective.expr += ...`-Erweiterung (Peak-Shaving) stattfand.
    #
    # Ursache (soweit ohne lauffaehiges Pyomo hier nachvollziehbar, siehe
    # Modulkopf-Hinweis): oemofs eigene Zielfunktion besteht bereits aus
    # MEHREREN separaten, effizient als `LinearExpression` aufgebauten
    # Kostenbeitraegen (je einer pro zeitreihenbasiertem Flow). Pyomo warnt in
    # der eigenen Doku explizit, dass die Listen einer `LinearExpression`
    # NICHT kopiert werden ("the lists that are passed to LinearExpression are
    # not copied") -- eine nachtraegliche Erweiterung dieser Zielfunktion per
    # `+=`/`+` (wie bisher in add_peak_shaving()/add_srl_optimiert()) kann
    # dadurch, je nach Pyomo-Version, bestehende Zeitreihen-Kostenterme beim
    # Zusammenfuehren verdoppeln statt sie nur anzuhaengen.
    #
    # Fix (zwei Teile):
    # 1. `add_peak_shaving()`/`add_srl_optimiert()` erweitern die Zielfunktion
    #    NICHT MEHR selbst, sondern geben ihren Zusatzkosten-Term nur noch
    #    zurueck -- hier werden ALLE aktiven Zusatzkosten gesammelt und NUR
    #    EIN EINZIGES MAL zusammengefuehrt (unabhaengig davon, ob 0, 1 oder 2
    #    Zusatzbloecke aktiv sind).
    # 2. Die eigentliche Zusammenfuehrung nutzt bewusst NICHT Pythons `+`/
    #    `sum()` (das genau den vermuteten LinearExpression-Fallstrick
    #    ausloest), sondern `pyomo.environ.quicksum(..., linear=False)` --
    #    erzwingt den generischen (nicht auf LinearExpression optimierten)
    #    Aufbau der kombinierten Zielfunktion und umgeht damit den
    #    vermuteten Mutations-Fallstrick.
    # Wirtschaftlich/inhaltlich AEQUIVALENT zum bisherigen Verhalten (dieselben
    # Kostenterme) -- BITTE trotzdem lokal mit genau dieser Datei erneut
    # laufen lassen und den neuen Solver-Log zurueckmelden (siehe
    # Modulkopf-Hinweis zur Testabdeckung), da diese Pyomo-Fallstrick-Klasse
    # in dieser Sandbox mangels Pyomo-Installation nicht nachstellbar ist.
    zusatzkosten_terme = []
    ...
    solve(om, solver, logfile=logfile)
